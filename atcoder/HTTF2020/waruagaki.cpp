...
int turn = 100;//robotの死なない保証ターン数(ちょっとウソ)
...
int dfs(point start, char dir, int depth) { //一直線に進んでgoal可能なら1000とか？死んだら-にしたいところ//もうn回先の行き止まりまで評価したい
    ...
    if (next_f == 'B' || next_f == m_inv[dir]) {
        int option[2];
        REP(i,2) {
            option[i] = dfs(start,ud_lr[m_ud_lr_idx[dir]][i],depth+D/4);
        }
